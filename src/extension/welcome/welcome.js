...
class WelcomeManager {
   ...
   */
  setupEventListeners() {
    this.elements.requestAccess.addEventListener("click", () =>
      this.requestMicrophoneAccess(),
    );
    this.elements.configureSettings.addEventListener("click", () =>
      this.openSettings(),
    );
  }
  ...
  async checkCurrentPermissions() {
    try {
      ...
      this.showStatus(
        getMessage("error_permissions") + ": " + error.message,
        "error",
      );
    }
  }
  ...
  async requestMicrophoneAccess() {
    ...
    const originalContent = Array.from(button.childNodes).map((n) =>
      n.cloneNode(true),
    );
    ...
    this.updateButtonState(
      button,
      [loadingIcon, document.createTextNode(" Requesting Access...")],
      true,
    );
  ...
  handleSuccessfulPermission(button) {
    ...
    this.updateButtonState(
      ...
      [
        successIcon,
        document.createTextNode(" " + getMessage("access_granted")),
      ],
      true,
    );
  ...
  updateFirstStep() {
    const firstStep =
      this.elements.nextSteps.querySelector("ol li:first-child");
    if (firstStep) {
